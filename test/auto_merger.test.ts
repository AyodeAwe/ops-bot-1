...
URL: https://github.com/rapidsai/cudf/pull/6775`,
  ...
  describe("validateMergeRequest", () => {
    ...
    test("reject when PR is authored by rapids-bot", async () => {
      ...
      const result = await autoMerger.validateMergeRequest(pr, "squash");
      ...
      expect(mockCreateComment).toHaveBeenCalledWith({
        ...
        repo: "cudf",
        issue_number: 1234,
        body: expect.stringContaining("AutoMerger commands (like `/merge` or `/merge nosquash`) cannot be used directly on PRs authored by RAPIDS bots.")
      });
    });
    ...
    test("allow nosquash merge on forward-merge branch", async () => {
      ...
      const result = await autoMerger.validateMergeRequest(pr, "merge");
  ...
  describe("getValidMergeComment", () => {
    ...
    test("should return null when no merge comments", async () => {
      ...
      const result = await autoMerger.getValidMergeComment(pr, comments);
      
      expect(result).toBeNull();
      expect(mockPaginate).toHaveBeenCalledTimes(0);
  ...
  describe("validateNoSquashMerge", () => {
    ...
    test("should implement lock-out after non-fixable validation failure", async () => {
      ...
      // Setup mocks for first validation that will return multiple PRs
      mockSearchIssuesAndPullRequests.mockResolvedValueOnce({
        data: {
      ...
      const secondResult = await autoMerger.validateNoSquashMerge(pr, commentsWithFailure);
    ...
    test("should allow retry after fixable validation failure", async () => {
      ...
      // Setup mocks for first validation
      mockSearchIssuesAndPullRequests.mockResolvedValueOnce({
        data: {
          items: [{ number: 1234 }]
        }
      ...
      const commentsWithFixableError = [{
      ...
      const secondResult = await autoMerger.validateNoSquashMerge(pr, commentsWithFixableError);
  ...
  describe("Merge Parameter Selection", () => {
    ...
    test("should set merge parameters for /merge nosquash command", async () => {
      ...
      // First paginate call: listComments for hasPermanentNosquashValidationFailure (empty comments)
      // Second paginate call: search for original bot PR
      mockSearchIssuesAndPullRequests.mockResolvedValueOnce({
        data: {
          items: [{ number: 1234 }]
        }
      });
      // Third call: get original PR details
      mockPullsGet.mockResolvedValueOnce({
        data: {
          ...makePullResponse().data,
          user: { login: "rapids-bot[bot]" },
      ...
      // Fourth call: Original PR commits
      mockPaginate.mockResolvedValueOnce([
        { sha: "commit1" },
        { sha: "commit2" }
      ]);
      
      // Fifth call: Current PR commits - include all original commits
      mockPaginate.mockResolvedValueOnce([
        { sha: "commit1" },
        { sha: "commit2" },
        { sha: "commit3" } // Extra commit is fine
